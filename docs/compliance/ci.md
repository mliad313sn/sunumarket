# Compliance checklist — Côte d'Ivoire (CI) — FR-41c / DC-12

Release-gate artifact. Status must be all ✅ or formally waived before public launch in Côte d'Ivoire.

| # | Item | Authority | Status | Notes |
|---|------|-----------|--------|-------|
| 1 | Déclaration / enregistrement DPA | ARTCI (Autorité de Régulation des Télécommunications/TIC) | ⬜ todo | Location data = donnée sensible : consentement par capture (FR-25, testé), chiffrement au repos, troncature après rétention (job testé). Dossier à déposer avant beta publique. |
...
| 4 | Liste des articles interdits (CI) | interne | ⬜ todo | Publier + brancher la modération (takedown livré et testé). |
...
| 8 | Méthodes de paiement conformes au marché | — | ✅ done | Wave, Orange Money, MTN MoMo, Moov Money, COD, PI-SPI, Carte. MANUAL_TRANSFER désactivé par défaut (DC-8.2). |
| 9 | Numérotation téléphonique validée (+225) | — | ✅ done | Regex pack, testée. |
